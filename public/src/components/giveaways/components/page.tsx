...
export const fetchGiveaways = async () => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_APP_API_URL}/giveaway/fetch-giveaway/`,
      {
        method: 'GET',
        headers: {
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 items-center justify-center py-4">
            {itemsGiveaway.length > 0 ? (
                ...
                <div key={itemGiveaway.id} className="relative flex flex-col items-center rounded-sm h-40 md:h-80 shadow-inner">
                    <Image width={1240} height={550} src={itemGiveaway.banner} className="absolute top-0 left-0 h-[calc(100%-16px)] w-full object-cover rounded-t-sm z-0" alt="" />
