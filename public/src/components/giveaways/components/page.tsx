        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 items-center justify-center py-4">
            {itemsGiveaway.length > 0 ? (
                ...
                <div key={itemGiveaway.id} className="relative flex flex-col items-center rounded-sm h-40 md:h-80 shadow-inner">
                    <Image width={1240} height={550} src={itemGiveaway.banner} className="absolute top-0 left-0 h-[calc(100%-16px)] w-full object-cover rounded-t-sm z-0" loader={imageLoader} alt="" />
                ...
                <div key={i} className="relative flex flex-col items-center rounded-sm h-40 md:h-80 shadow-inner">
                    <Image width={1240} height={550} src={"https://flowbite.com/docs/images/carousel/carousel-1.svg"} className="absolute top-0 left-0 h-[calc(100%-16px)] w-full object-cover rounded-t-sm z-0" loader={imageLoader} alt="" />
                    <div className="absolute top-0 h-2 w-full flex flex-row items-center gap-x-1">
                        <div className='absolute top-0 h-full w-full bg-gradient-to-r from-lime-500 to-red-500 transition-all duration-200'/>
                    </div>
                    <span className="absolute bottom-5 right-2 flex items-center justify-between gap-x-2 bg-gray-800 hover:bg-gray-900  border-slate-950 transition-colors duration-300 px-4 py-2 rounded border-b-2 cursor-not-allowed">
                        <span className='text-white font-semibold text-md'><AiOutlineShoppingCart /></span>
                        <span className="block text-white shadow-inner text-xs uppercase font-semibold">
                            Comprar
                        </span>
