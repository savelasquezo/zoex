...
const TicketsMiniLotteryModal: React.FC<SessionModal> = ({ closeModal, session  }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!aviableTickets.includes(ticket)) {
      setError('¡Lamentablemente ya ha sido Adquirido!/n Elije otro');
            ...
            <div className='w-full flex flex-col justify-start items-center gap-y-2 my-12 lg:w-3/5 lg:my-10'>
              ...
              <div className='relative w-full h-36 gap-y-0.5 px-4 my-4 hidden lg:flex lg:flex-col lg:justify-start'>
                ...
                <div className='absolute -bottom-8 right-4'>
                  <span className='relative h-full w-full flex items-center'>
                    <Image width={256} height={256} src={"/assets/image/glump.webp"} alt="" className="w-auto h-20"/>
                    <p className='absolute text-center text-3xl z-20 right-1/4 font-semibold text-slate-800 top-4'>${minilottery.price}</p>
                    <p className='absolute text-center z-20 right-5 font-semibold text-slate-800 bottom-3'>USD</p>
                  </span>
                </div>
              </div>
