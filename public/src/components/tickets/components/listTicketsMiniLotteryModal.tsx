...
export const fetchMiniLotteryTickets = async (accessToken: any) => {
    try {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_APP_API_URL}/app/minilottery/fetch-all-minilottery-tickets/`,
            {
                method: 'GET',
                headers: {
...
const ListTicketsMiniLotteryModal: React.FC<SessionModal>  = ({closeModal, session }) => {
    ...
    const [minilotteryTickets, setMiniLotteryTickets] = useState<MiniLotteryTicketDetails[]>([]);
    ...
    useEffect(() => {
      if (session) {
        ...
        const fetchData = async () => {
          await fetchMiniLotteryTickets(accessToken)
          .then((data) => {
            setMiniLotteryTickets(data);
          })
          .catch((error) => {
    ...
    const pageCount = Math.ceil(minilotteryTickets.length/ticketsPerPage);
